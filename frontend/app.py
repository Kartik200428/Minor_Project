...
st.set_page_config(
    page_title="Multi-Agent Framework for Spam and Phishing Analysis",
    page_icon="🤖",
    layout="wide",
)

st.title("🤖 Multi-Agent Framework for Spam and Phishing Analysis")
st.write(
    ...
    if analyze_clicked:
        if not subject.strip() and not body.strip():
            ...
            if result is not None:
                ...
                # Fetch spam classification result
                try:
                    spam_resp = requests.post(
                        f"{backend_url}/classify_spam",
                        json={"text": body},
                        timeout=30,
                    )
                    spam_resp.raise_for_status()
                    spam_result = spam_resp.json()

                    classification = spam_result.get("classification", "Unknown")
                    probability = spam_result.get("probability", 0.0)
                except Exception as e:
                    classification = "Error"
                    probability = 0.0
                    st.error(f"Error during spam classification: {e}")

                # Add spam classification result to the summary
                cols = st.columns(2)
                with cols[0]:
                    st.metric("Spam Classification", classification)
                with cols[1]:
