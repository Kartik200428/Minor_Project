...
# --------- Spam classification endpoints ---------


# Adjust the paths to be relative to the server's working directory
try:
    vectorizer = pickle.load(open("data/processed/vectorizer.pkl", "rb"))
    svc_model = pickle.load(open("data/processed/svc_model.pkl", "rb"))
except Exception as e:
    raise HTTPException(status_code=500, detail=f"Failed to load spam classifier: {e}")
...
@app.post("/classify_spam", response_model=SpamClassificationResponse)
def classify_spam(req: SpamClassificationRequest) -> SpamClassificationResponse:
    """
    Classify an email as spam or ham using the pre-trained SVC model.
    """
    try:
        transformed_text = vectorizer.transform([req.text]).toarray()
        prediction = svc_model.predict(transformed_text)[0]
        probability = svc_model.predict_proba(transformed_text)[0][1]
        classification = "Spam" if prediction == 1 else "Ham"
        return SpamClassificationResponse(classification=classification, probability=probability)
    except Exception as e:
